...
Return JSON array of vulnerabilities found. Each entry MUST include: line (integer line number, not null), type (string), severity (high/medium/low), description (string - detailed explanation up to 4 sentences), location (file path).
...
[
  {{
    ...
    "severity": "medium",
    "description": "No password complexity requirements detected in authentication logic at line 42. This vulnerability allows users to create weak passwords that are easily guessable or brute-forceable. Attackers could exploit this to gain unauthorized access to user accounts by trying common passwords or dictionary attacks. The code should implement password policies requiring minimum length, complexity (uppercase, lowercase, numbers, special characters), and prevent common weak passwords.",
    "location": "{file_path}"
  }}
]

If no vulnerabilities found, return empty array []. Be specific about what authentication issues you identify and ALWAYS include the line number. Provide detailed descriptions (up to 4 sentences) explaining the vulnerability, its risks, potential impact, and remediation."""
    else:
        user_prompt = f"""Analyze this file for authentication vulnerabilities:
...
If no vulnerabilities found, return empty array []. Be specific about what authentication issues you identify. Provide detailed descriptions (up to 4 sentences) explaining the vulnerability, its risks, potential impact, and remediation."""
